...
import com.example.mlab_assesment.model.mapper.BookMapper;
...
import com.example.mlab_assesment.service.EmailService;
import com.example.mlab_assesment.service.crud.BookEntityService;
...
public class BookServiceImpl extends BaseService implements BookService {
    ...
    public UserResponse submitBooks(BookSubmitRequest dto) {
        ...
        submitBooks(user, dto.getBookIds());

        emailService.sendEmail(
                user.getEmail(),
                "Book Submitted",
                String.format("Book submitted by: %s", user.getFullName()));

        List<BookMeta> newMetaList = metaEntityService.findMetaInBooks(user.getBooks());
        return userMapper.mapToDto(user, newMetaList);
    }
    ...
    private void submitBooks(User userEntity, Set<Long> bookIds){
